...
class VerbsAgent(Thread):
    ...
    def generate_rich_analysis_mt(self, row_nbr=5):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            future_to_verb = {
                executor.submit(
                    self._generate_verb_table,
                    verb,
                    lemma,
                    verb_meaning_cache,
                    verb_conjugation_cache,
                    row_nbr,
                ): verb
    ...
    def _generate_verb_table(
        self, verb, lemma, verb_meaning_cache, verb_conjugation_cache, row_nbr
    ):
        ...
        if conj_tables is not None:
            ...
            if "None" not in conj_tables.values:
                ...
                conj_tables = verb_conjugation_cache.get(lemma, {}).get(
                    "indicative_active"
                )
                if conj_tables is not None:
                    conj_tables.fillna(value="None", inplace=True)
